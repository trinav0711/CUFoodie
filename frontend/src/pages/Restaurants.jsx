...
export default function Restaurants() {
  ...
  async function fetchList() {
    try {
      ...
      // Filter by minRating if backend does not support it yet
      let filtered = json;
      if (minRating) {
        filtered = json.filter(r => r.avg_rating >= parseFloat(minRating));
      }
          ...
          <tbody>
            ...
            {data.map(r => (
              <tr key={r.name}>
                ...
                <td>{r.avg_rating !== undefined && r.avg_rating !== null ? r.avg_rating.toFixed(1) : '—'}</td>
                <td>
                  <Link to={`/restaurant/${r.restaurant_id}`} className="link">View Menu</Link>
                </td>
              </tr>
            ))}
          </tbody>
